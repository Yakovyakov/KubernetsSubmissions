...
kubectl run  -it --rm  --restart=Never --image postgres psql-for-debugging -n project --   sh -c "PGPASSWORD='$DB_PASSWORD' PAGER=cat psql -q -h '$DB_HOST' -p '$DB_PORT' -U '$DB_USER' -d '$DB_NAME' -c 'DELETE FROM todos;' -v ON_ERROR_STOP=1"
